...
export class DirectMappedCache {
	...
	public access(memoryBlock: number): AccessResult {
		const validationError = validateMemoryBlock(memoryBlock, this.config.mainMemoryBlockCount);
		...
		const isHit = line.valid && line.memoryBlock === memoryBlock && line.tag === tag;
	...
	private createEmptyLines(): CacheLine[] {
		return Array.from({ length: this.config.cacheBlockCount }, (_, lineIndex) => ({
			lineIndex,
			valid: false,
			memoryBlock: null,
			tag: null,
